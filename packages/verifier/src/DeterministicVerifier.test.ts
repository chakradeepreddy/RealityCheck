...
describe('DeterministicVerifier', () => {
  it('Fixture A — HONEST (SUPPORTED)', () => {
    ...
    const result = DeterministicVerifier.verifyBoundary(spec, analysis, spec.testConditions.cartSubtotalTarget || 0);
  ...
  it('Fixture B — CONTRADICTED (Observed higher)', () => {
    ...
    expect(result.reason).toContain('Claimed threshold is 999, but observed transition begins at 1050');
  });
  ...
  it('Returns INCONCLUSIVE for missing claimed threshold', () => {
    ...
    const result = DeterministicVerifier.verifyBoundary(spec, analysis, spec.testConditions.cartSubtotalTarget as unknown as number);
  ...
  it('accepts QUANTITY_DISCOUNT boundaryType', () => {
  ...
  it('Repeated identical inputs give same deterministic result', () => {
    ...
    const result1 = DeterministicVerifier.verifyBoundary(spec, analysis, spec.testConditions.cartSubtotalTarget || 0);
    const result2 = DeterministicVerifier.verifyBoundary(spec, analysis, spec.testConditions.cartSubtotalTarget || 0);
